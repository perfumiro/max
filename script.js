...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const logoAreaImageUrl = 'assets/Herosectionphotos/logo area.png';
    ...
    const productDetailOverrides = {
        ...
        'guerlain l homme id al l intense eau de parfum': {
            ...
            fr: {
                ...
                notes: [
                    ...
                    { title: 'INTENSIT— LAQUÉE NOIRE', text: 'Un sillage luxueux et intensément stylé, reflét— par le design de flacon noir facetté.' }
                ]
            },
            images: [
        ...
        'azzaro the most wanted eau de parfum intense': {
            ...
            fr: {
                ...
                notes: [
                    ...
                    { title: 'BOIS AMBRÉS', text: 'Un fond bois— lisse qui donne une présence durable.' }
                ]
            },
            images: [
        ...
        'dior sauvage eau de parfum': {
            ...
            fr: {
                ...
                notes: [
                    ...
                    { title: 'BOIS AMBRÉS', text: 'Un fond bois— et ambr— qui laisse un sillage durable et mémorable.' }
                ]
            },
            images: [
        ...
        'dior homme intense eau de parfum': {
            ...
            fr: {
                ...
                notes: [
                    ...
                    { title: 'CÈDRE', text: 'Un fond bois— propre qui donne structure et longue durabilité.' }
                ]
            },
            images: [
        ...
        'yves saint laurent myslf eau de parfum': {
            ...
            notes: [
                ...
                {
                    title: 'PATCHOULI & AMBROFIXÉ',
                    text: 'A sensual woody base of Indonesian patchouli and Ambrofixé creates texture and modern depth.'
                }
            ],
            fr: {
                ...
                notes: [
                    ...
                    { title: 'PATCHOULI & AMBROFIXÉ', text: 'Une base boisée sensuelle de patchouli indonésien et d\'Ambrofixé crée texture et profondeur moderne.' }
                ]
            },
            images: [
        ...
        'gucci guilty absolu de parfum pour homme': {
            ...
            fr: {
                ...
                notes: [
                    ...
                    { title: 'CONTRASTE PIMENT ROUGE', text: 'Le piment rouge ajoute un contraste épic— vif qui aiguise le parfum et approfondit son caractère.' }
                ]
            },
            images: [
        ...
        'prada luna rossa ocean eau de parfum': {
            ...
            fr: {
                ...
                longDescription: 'PRADA Luna Rossa Ocean Eau de Parfum associe sophistication intense et sensualité grâce au contraste entre technologie innovante et ingrédients naturels d\'exception. Le parfum s\'ouvre sur un éclat vivifiant d\'essence de pamplemousse, évolue vers la vibration boisée de l\'encens, puis se pose sur un accord de vanille riche, créant un sillage — la fois frais, profond et durable.',
                notes: [
        ...
        'prada paradigme eau de parfum': {
            ...
            fr: {
                ...
                notes: [
                    ...
                    { title: 'BAUME DU PÉROU, BENJOIN & BOIS DE GAÏAC', text: 'Un fond bois— balsamique de baume du Pérou, de benjoin et de bois de gaïac apporte une chaleur douce et un sillage posé de longue durée.' }
                ]
            },
            images: [
    ...
    const injectFirestoreProductCards = async (pricesById) => {
        ...
        try {
            ...
            snap.forEach(docSnap => {
                ...
                const sizeBadgesHtml = visibleSizes.map((sz, i) =>
                ...
                const _stockHtml = _stockLeft === null ? ''
                    ...
                    : _stockLeft <= 5
                        ? `<span style="display:inline-block;margin-bottom:8px;font-size:10px;font-weight:700;color:#dc2626;background:rgba(220,38,38,0.08);border:1px solid rgba(220,38,38,0.2);border-radius:6px;padding:3px 9px;"><i class="fas fa-fire" style="margin-right:4px;font-size:9px"></i>${_isFr ? `Plus que ${_stockLeft} !` : `Only ${_stockLeft} left!`}</span>`
                    : _stockLeft <= 15
                ...
                if (discoverGrid && !discoverGrid.querySelector(`[data-id="${CSS.escape(slug)}"][data-firestore-product="true"]`)) {
                    ...
                    ).join('') + (_dHiddenCount > 0 ? `<span class="text-[10px] font-medium border border-dashed border-gray-300 text-gray-400 px-2 py-1 rounded">+${_dHiddenCount}</span>` : '');
    ...
    const buildProductReviewSet = (productName, productOverride) => {
        ...
        const openerTemplates = currentLanguage === 'fr' ? [
            'Le %ACCENT% est ce qui m\'a immédiatement accroché — trés naturel et bien intégré.',
            'Honnêtement, le %ACCENT% est bien mieux que ce — quoi je m\'attendais pour ce prix.',
            ...
            'Le %ACCENT% donne au parfum une vraie personalité, pas générique du tout.',
        ...
        const closerTemplates = currentLanguage === 'fr' ? [
            'La livraison — Casablanca était rapide et l\'emballage était impeccable. Je repasserai commande.',
            'ça tient facilement 8 — 10 heures sur moi, ce qui est rare — ce prix.',
            'L\'emballage — la réception était soigné et le flacon correspond exactement aux photos.',
            ...
            'Commande reçue en 2 jours, parfum authentique, trés satisfied de l\'expérience globale.',
    ...
    // ── Shared wishlist auth modal ───────────────────────────────
    const showWishlistAuthModal = (base = 'pages/') => {
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                <div id="wishlistAuthBackdrop" style="position:absolute;inset:0;background:rgba(0,0,0,0.6);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);transition:opacity 0.3s;"></div>
                <div class="wl-modal-card" style="position:relative;background:#fff;border-radius:24px;padding:2.8rem 2.2rem 2.2rem;max-width:380px;width:calc(100% - 32px);box-shadow:0 32px 80px rgba(0,0,0,0.22),0 0 0 1px rgba(255,255,255,0.06) inset;text-align:center;animation:wishlistModalIn 0.38s cubic-bezier(0.22,1,0.36,1);overflow:hidden;">
                    <button id="wishlistAuthClose" type="button" aria-label="Close" style="position:absolute;top:16px;right:18px;background:none;border:none;cursor:pointer;font-size:13px;color:#9ca3af;line-height:1;transition:color 0.2s;width:32px;height:32px;display:flex;align-items:center;justify-content:center;border-radius:50%;transition:all 0.2s;">&#x2715;</button>
                    <div class="wl-heart-icon" style="width:68px;height:68px;border-radius:50%;background:linear-gradient(135deg,#fff0f0,#ffe0e6);display:flex;align-items:center;justify-content:center;margin:0 auto 1.3rem;box-shadow:0 8px 24px rgba(231,76,76,0.15);animation:wishlistHeartPulse 1.8s ease-in-out infinite;">
                        <i class="fas fa-heart" style="font-size:26px;color:#e74c4c;"></i>
                    </div>
                    <h3 style="font-size:1.35rem;font-weight:800;color:#111827;margin:0 0 0.5rem;font-family:'Playfair Display',serif;letter-spacing:-0.01em;">Save to Wishlist</h3>
                    <p style="font-size:0.88rem;color:#6b7280;margin:0 0 1.8rem;line-height:1.6;">Sign in to save your favourite fragrances and access your wishlist from any device.</p>
                    <a id="wishlistAuthSignIn" href="${base}login.html" class="wl-btn-primary" style="display:flex;align-items:center;justify-content:center;gap:8px;background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;font-size:0.82rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;padding:14px 0;border-radius:14px;text-decoration:none;margin-bottom:10px;transition:all 0.25s;box-shadow:0 4px 16px rgba(0,0,0,0.18);"><i class="fas fa-right-to-bracket" style="font-size:14px;"></i> Sign In</a>
                    <a id="wishlistAuthRegister" href="${base}register.html" class="wl-btn-secondary" style="display:flex;align-items:center;justify-content:center;gap:8px;background:#fff;color:#111827;font-size:0.82rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;padding:13px 0;border-radius:14px;text-decoration:none;border:1.5px solid #e5e7eb;transition:all 0.25s;"><i class="fas fa-user-plus" style="font-size:13px;color:#6b7280;"></i> Create Account</a>
                    <p style="font-size:0.72rem;color:#9ca3af;margin:1.4rem 0 0;line-height:1.4;"><i class="fas fa-lock" style="font-size:9px;margin-right:3px;"></i> Your data is secure and never shared</p>
                </div>
            `;
            ...
            if (!style) {
                ...
                style.textContent = [
                    '@keyframes wishlistModalIn{from{opacity:0;transform:translateY(20px) scale(0.95);}to{opacity:1;transform:translateY(0) scale(1);}}',
                    '@keyframes wishlistHeartPulse{0%,100%{transform:scale(1);}50%{transform:scale(1.08);}}',
                    '#wishlistAuthClose:hover{background:rgba(0,0,0,0.06);color:#374151;}',
                    '.wl-btn-primary:hover{box-shadow:0 6px 24px rgba(0,0,0,0.28);transform:translateY(-1px);}',
                    '.wl-btn-secondary:hover{background:#f9fafb;border-color:#d1d5db;transform:translateY(-1px);}',
                ].join('');
        ...
        const card = modal.querySelector('.wl-modal-card');
    ...
    const resetCarouselPositions = () => {
        ['productCarousel', 'newArrivalsCarousel', 'carousel2026'].forEach((id) => {
            const el = document.getElementById(id);
            if (el) el.scrollTo({ left: 0, behavior: 'instant' });
        });
    };
    requestAnimationFrame(() => requestAnimationFrame(resetCarouselPositions));
    // Final fallback: after all async Firestore injections & image loads finish,
    // force carousels back to position 0 one more time.
    ...
    const initSearchOverlay = () => {
        ...
        const norm = (s) => String(s || '').toLowerCase()
            .normalize('NFD').replace(/[\u0300-\u036f]/g, '')   // strip accents: é?e, é?a

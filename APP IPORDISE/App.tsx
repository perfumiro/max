...
function LaunchIntro({onFinish}:{onFinish:()=>void}) {
  ...
  return <Animated.View accessibilityRole="none" accessibilityLabel="IPORDISE is opening" style={[styles.launchIntro,{opacity:backdrop}]}>
    ...
    <Animated.View style={[styles.launchIdentity,{opacity:wordmark,transform:[{translateY:wordmark.interpolate({inputRange:[0,1],outputRange:[10,0]})},{scale:wordmark.interpolate({inputRange:[0,1],outputRange:[.965,1]})}]}]}>
      <Svg width={238} height={54} viewBox="0 0 238 54">
        <SvgText x="119" y="39" textAnchor="middle" fill="#fff" fontFamily="Times New Roman, serif" fontSize="38" fontWeight="700" letterSpacing="-.7">IPORDISE</SvgText>
      </Svg>
      <Animated.View style={[styles.launchSignature,{opacity:signature,transform:[{scaleX:signature}]}]}>
        <Svg width={206} height={28} viewBox="0 0 206 28">
          <Path d="M5 4 C42 24 86 25 132 12 C155 5.5 179 8 201 16" fill="none" stroke="#e4003b" strokeWidth="3.2" strokeLinecap="round"/>
        </Svg>
      </Animated.View>
...
function PremiumHomeFeed({layout,products,visibleProducts,bestsellerIds,query,activeFilter,catalogSynced,catalogError,homeConfig,onFilter,onSearch,onBrand,onProduct,onCollection}:{layout:ReturnType<typeof useResponsiveLayout>;products:Product[];visibleProducts:Product[];bestsellerIds:string[];query:string;activeFilter:string;catalogSynced:boolean;catalogError:boolean;homeConfig:HomeConfig;onFilter:(filter:string)=>void;onSearch:(query:string)=>void;onBrand:(brand:string)=>void;onProduct:(product:Product)=>void;onCollection:(collection:(typeof discoveryLinks)[number])=>void}) {
  ...
  const offers=useMemo(()=>homeProducts.filter(product=>product.oldPrice||product.filters.includes('offers')),[homeProducts]);
  ...
  const renderSection=({item}:{item:HomeFeedSection})=>{
    ...
    if(item==='offers')return <HomeProductRail eyebrow="CURRENT PROMOTIONS" title="Today’s offers" subtitle="Real reductions from the live catalogue." products={offers.slice(0,10)} layout={layout} onOpen={onProduct} onViewAll={()=>onFilter('offers')}/>;
...
function SupportChatCard() {
  ...
  return <View style={styles.supportChatCard}><View style={styles.supportChatIntro}><View style={styles.supportChatIcon}><Ionicons name="chatbubbles-outline" size={21} color="#fff"/></View><View style={{flex:1}}><Text style={styles.supportChatEyebrow}>MESSAGE THE BOUTIQUE</Text><Text style={styles.supportChatTitle}>Start a private conversation.</Text><Text style={styles.supportChatCopy}>Your message is delivered securely to the IPORDISE staff inbox.</Text></View></View><View style={[styles.supportTopicRow,layout.compact&&{flexWrap:'wrap'}]}>{['General support','Order help','Product advice'].map(topic=><Pressable accessibilityRole="button" accessibilityState={{selected:subject===topic}} key={topic} onPress={()=>setSubject(topic)} style={[styles.supportTopic,layout.compact&&{flexBasis:'47%'},subject===topic&&styles.supportTopicActive]}><Text style={[styles.supportTopicText,subject===topic&&styles.supportTopicTextActive]}>{topic}</Text></Pressable>)}</View><View style={[styles.supportFieldRow,layout.compact&&{flexDirection:'column',gap:0}]}><TextInput accessibilityLabel="Your name" autoCapitalize="words" value={name} onChangeText={setName} placeholder="Your name" placeholderTextColor="#91867f" style={[styles.supportField,styles.supportFieldHalf,layout.compact&&{width:'100%',flex:0}]}/><TextInput accessibilityLabel="Email address" autoCapitalize="none" autoCorrect={false} keyboardType="email-address" value={email} onChangeText={setEmail} placeholder="Email address" placeholderTextColor="#91867f" style={[styles.supportField,styles.supportFieldHalf,layout.compact&&{width:'100%',flex:0}]}/></View><TextInput accessibilityLabel="Order number, optional" autoCapitalize="characters" value={orderNumber} onChangeText={setOrderNumber} placeholder="Order number (optional)" placeholderTextColor="#91867f" style={styles.supportField}/><TextInput accessibilityLabel="Support message" multiline textAlignVertical="top" value={message} onChangeText={setMessage} placeholder="How can our team help?" placeholderTextColor="#91867f" style={[styles.supportField,styles.supportMessageField]}/>{error?<Text accessibilityRole="alert" style={styles.supportChatError}>{error}</Text>:null}<Pressable accessibilityRole="button" accessibilityState={{busy:loading,disabled:loading}} disabled={loading} onPress={createConversation} style={({pressed})=>[styles.supportSubmitButton,loading&&styles.supportSendButtonDisabled,pressed&&styles.pressed]}><Text style={styles.supportSubmitText}>{loading?'SENDING SECURELY…':'SEND TO IPORDISE CARE'}</Text><Ionicons name="arrow-forward" size={16} color="#fff"/></Pressable><View style={styles.supportPrivacy}><Ionicons name="lock-closed-outline" size={11} color="#82766f"/><Text style={styles.supportPrivacyText}>Encrypted in transit. Never share passwords or payment-card details in chat.</Text></View></View>;
}
...
const styles:Record<string,any> = StyleSheet.create({
  launchIntro:{...StyleSheet.absoluteFillObject,zIndex:10000,alignItems:'center',justifyContent:'center',overflow:'hidden',backgroundColor:'#030303'},launchAmbientGlow:{position:'absolute',width:300,height:300,borderRadius:150,backgroundColor:'rgba(215,25,63,.035)',shadowColor:'#d7193f',shadowOpacity:.12,shadowRadius:80,shadowOffset:{width:0,height:0}},launchIdentity:{width:250,alignItems:'center',justifyContent:'center'},launchSignature:{height:28,marginTop:-9,alignItems:'center',justifyContent:'center'},launchCaption:{marginTop:17,color:'rgba(255,255,255,.52)',fontSize:7,lineHeight:11,fontWeight:'800',letterSpacing:2.7},
  ...
  supportPage:{flex:1,backgroundColor:'#f6f3f1'},supportPageScroll:{paddingTop:22,paddingBottom:28},supportPageContainer:{maxWidth:620},carePageHeader:{borderRadius:21,backgroundColor:'#fff',borderWidth:1,borderColor:'#e6ddd8',padding:19},carePageTop:{height:42,flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:17},careBack:{width:40,height:40,borderRadius:20,backgroundColor:'#f5f1ef',alignItems:'center',justifyContent:'center'},careSecure:{width:40,height:40,borderRadius:20,backgroundColor:'#edf8f1',alignItems:'center',justifyContent:'center'},carePageEyebrow:{fontSize:7,fontWeight:'900',letterSpacing:1.5,color:RED},carePageTitle:{fontFamily:'serif',fontSize:29,lineHeight:34,fontWeight:'700',color:'#171310',marginTop:4},carePageSubtitle:{fontSize:10.5,lineHeight:16,color:'#756a64',marginTop:5,maxWidth:420},
  ...
  supportChatCard:{marginTop:20,borderRadius:22,backgroundColor:'#fff',borderWidth:1,borderColor:'#e3dad5',padding:17,shadowColor:'#2a1915',shadowOpacity:.08,shadowRadius:15,shadowOffset:{width:0,height:7},elevation:3},supportChatIntro:{flexDirection:'row',alignItems:'center',gap:12},supportChatIcon:{width:44,height:44,borderRadius:15,backgroundColor:RED,alignItems:'center',justifyContent:'center'},supportChatEyebrow:{fontSize:6.5,lineHeight:10,fontWeight:'900',letterSpacing:1.4,color:RED},supportChatTitle:{fontFamily:'serif',fontSize:20,lineHeight:24,fontWeight:'700',color:'#171310',marginTop:2},supportChatCopy:{fontSize:9,lineHeight:13,color:'#7b7069',marginTop:2},supportTopicRow:{flexDirection:'row',gap:6,marginTop:17},supportTopic:{flex:1,minHeight:35,borderRadius:12,borderWidth:1,borderColor:'#ddd5d0',backgroundColor:'#faf8f7',paddingHorizontal:5,alignItems:'center',justifyContent:'center'},supportTopicActive:{backgroundColor:'#171310',borderColor:'#171310'},supportTopicText:{fontSize:7.5,fontWeight:'800',color:'#6f6660',textAlign:'center'},supportTopicTextActive:{color:'#fff'},supportFieldRow:{flexDirection:'row',gap:8},supportField:{height:49,borderRadius:13,borderWidth:1,borderColor:'#d9d1cc',backgroundColor:'#fcfbfa',paddingHorizontal:13,fontSize:11,color:'#211c19',marginTop:9},supportFieldHalf:{flex:1,minWidth:0},supportMessageField:{height:94,paddingTop:12,paddingBottom:12},supportChatError:{fontSize:9,lineHeight:13,fontWeight:'700',color:RED,marginTop:8},supportSubmitButton:{height:50,borderRadius:25,backgroundColor:RED,marginTop:12,paddingHorizontal:18,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},supportSubmitText:{fontSize:7.5,fontWeight:'900',letterSpacing:1.15,color:'#fff'},supportPrivacy:{marginTop:10,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:5},supportPrivacyText:{fontSize:7,lineHeight:10,color:'#8a7f79',textAlign:'center'},supportChatHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:10,paddingBottom:13,borderBottomWidth:1,borderBottomColor:'#eee8e4'},supportChatStatus:{height:25,borderRadius:13,backgroundColor:'#edf8f1',paddingHorizontal:8,flexDirection:'row',alignItems:'center',gap:5},supportChatStatusDot:{width:5,height:5,borderRadius:3,backgroundColor:'#2c9b5c'},supportChatStatusText:{fontSize:5.5,fontWeight:'900',letterSpacing:.8,color:'#176b43'},supportThread:{gap:8,paddingVertical:14},supportBubble:{maxWidth:'86%',borderRadius:15,paddingHorizontal:12,paddingVertical:10},supportBubbleCustomer:{alignSelf:'flex-end',backgroundColor:'#171310',borderBottomRightRadius:4},supportBubbleStaff:{alignSelf:'flex-start',backgroundColor:'#f5eeee',borderBottomLeftRadius:4},supportBubbleSender:{fontSize:5.5,fontWeight:'900',letterSpacing:1,color:'rgba(255,255,255,.6)'},supportBubbleSenderStaff:{color:RED},supportBubbleText:{fontSize:10,lineHeight:15,color:'#29211d',marginTop:3},supportBubbleTextCustomer:{color:'#fff'},supportBubbleTime:{fontSize:5.8,color:'#8b7f78',marginTop:5},supportBubbleTimeCustomer:{color:'rgba(255,255,255,.46)'},supportReplyRow:{minHeight:52,borderRadius:16,borderWidth:1,borderColor:'#d9d1cc',backgroundColor:'#fcfbfa',paddingLeft:12,paddingRight:5,paddingVertical:5,flexDirection:'row',alignItems:'center',gap:7},supportReplyInput:{flex:1,minWidth:0,maxHeight:90,fontSize:11,color:'#211c19',paddingVertical:5},supportSendButton:{width:40,height:40,borderRadius:14,backgroundColor:RED,alignItems:'center',justifyContent:'center'},supportSendButtonDisabled:{opacity:.45},supportChatFooter:{marginTop:10,flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:9},supportChatSecure:{flex:1,fontSize:6.5,lineHeight:10,color:'#8a7f79'},supportNewConversation:{fontSize:7,fontWeight:'900',color:RED},adminPortalCard:{minHeight:94,marginTop:12,borderRadius:18,backgroundColor:'#e9e5e2',borderWidth:1,borderColor:'#d9d2cd',padding:14,flexDirection:'row',alignItems:'center',gap:11},adminPortalIcon:{width:43,height:43,borderRadius:14,backgroundColor:'#171310',alignItems:'center',justifyContent:'center'},adminPortalCopy:{flex:1,minWidth:0},adminPortalEyebrow:{fontSize:5.8,fontWeight:'900',letterSpacing:1.15,color:RED},adminPortalTitle:{fontFamily:'serif',fontSize:17,lineHeight:21,fontWeight:'700',color:'#171310',marginTop:2},adminPortalText:{fontSize:7.8,lineHeight:11.5,color:'#736963',marginTop:2},adminPortalButton:{width:38,height:38,borderRadius:19,backgroundColor:'#fff',borderWidth:1,borderColor:'#d7cfca',alignItems:'center',justifyContent:'center'},
